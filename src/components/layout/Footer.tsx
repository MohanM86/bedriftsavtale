    ...
    <footer className="bg-[var(--navy)]">
      <div className="container-site pt-20 pb-16">
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-10 pb-14 border-b border-white/10">
          <div className="max-w-sm">
            ...
            <div className="mt-6 pt-5 border-t border-white/[.07] flex items-center gap-2">
              ...
              <a href="https://it-firma.no" target="_blank" rel="noopener noreferrer" className="text-sm font-bold text-teal-300/60 hover:text-teal-300 transition-colors tracking-tight">
                IT-Firma
              </a>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-x-8 gap-y-12 pt-14">
          <div className="col-span-2 md:col-span-3 lg:col-span-2">
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-2.5">
              {footerCategories.map(link => (
                <Link key={link.href} href={link.href} className="text-sm text-white/55 hover:text-white transition-colors leading-snug">
                  {link.label}
                </Link>
              ))}
          ...
          <div>
            <h3 className="text-sm font-extrabold tracking-[.1em] uppercase text-teal-400 mb-5 pb-3 border-b border-white/10">Guider</h3>
            <ul className="space-y-2.5">
              {footerGuides.map(link => (
                <li key={link.href}>
                  <Link href={link.href} className="text-sm text-white/55 hover:text-white transition-colors leading-snug block">{link.label}</Link>
                </li>
              ))}
              <li><Link href="/guide" className="text-sm font-bold text-teal-400 hover:text-teal-300 transition-colors">Se alle guider →</Link></li>
            </ul>
          </div>

          <div>
            <h3 className="text-sm font-extrabold tracking-[.1em] uppercase text-teal-400 mb-5 pb-3 border-b border-white/10">Bransjer</h3>
            <ul className="space-y-2.5 mb-8">
              ...
              <li><Link href="/bransje" className="text-sm font-bold text-teal-400 hover:text-teal-300 transition-colors">Alle bransjer →</Link></li>
            </ul>
            <h3 className="text-sm font-extrabold tracking-[.1em] uppercase text-teal-400 mb-5 pb-3 border-b border-white/10">Din bedrift</h3>
            <ul className="space-y-2.5">
          ...
          <div>
            <h3 className="text-sm font-extrabold tracking-[.1em] uppercase text-teal-400 mb-5 pb-3 border-b border-white/10">Informasjon</h3>
            <ul className="space-y-2.5 mb-8">
              {info.map(link => (
                <li key={link.href}>
                  <Link href={link.href} className="text-sm text-white/55 hover:text-white transition-colors leading-snug block">{link.label}</Link>
            ...
            <div className="bg-teal-500/15 border border-teal-500/25 rounded-2xl p-5">
              ...
              <p className="text-xs text-white/50 leading-relaxed mb-4">Ring oss for gratis og uforpliktende veiledning om bedriftsavtaler.</p>
              <a href="tel:22000000" className="block bg-teal-500 hover:bg-teal-400 text-white text-sm font-bold text-center py-3 rounded-xl transition-colors">
                Ring 22 00 00 00
              </a>
            </div>
          </div>
      ...
      <div className="border-t border-white/[.07]">
        <div className="container-site py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-sm text-white/25">
            © {new Date().getFullYear()} Bedriftsavtale.no, En tjeneste fra{" "}
            <a href="https://it-firma.no" target="_blank" rel="noopener noreferrer" className="text-white/40 hover:text-white/60 transition-colors">IT-Firma</a>
          </p>
          <div className="flex items-center gap-2 flex-wrap justify-center">
            {["40+ bedriftsavtaler", "Norges største", "AEO-optimalisert", "GDPR-compliant"].map(b => (
              <span key={b} className="text-xs font-semibold tracking-wide uppercase text-white/25 border border-white/10 rounded-full px-3 py-1">{b}</span>
            ))}
          </div>
        </div>
      </div>
