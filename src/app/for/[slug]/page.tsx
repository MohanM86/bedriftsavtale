...
export default function CompanySizeDetailPage({ params }: Props) {
  ...
  return (
    <>
        ...
        <div className="grid lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-teal-50 border-l-4 border-teal-500 rounded-r-xl p-5">
              <p className="text-xs font-bold tracking-[.12em] uppercase text-teal-500 mb-2">Tilpasset din størrelse</p>
              <p className="text-sm text-[var(--text)] leading-relaxed">Her er avtaletypene som erfaringsmessig gir størst verdi for {size.title.toLowerCase()}.</p>
            </div>
            <div>
                ...
                {sizeCats.map(cat => (
                  <Link key={cat.slug} href={`/${cat.slug}`} className="card card-hover flex items-start gap-3.5 p-4 group">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[var(--muted)] mt-0.5 line-clamp-2 leading-relaxed">{cat.description.slice(0, 70)}…</p>
                    </div>
                  </Link>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {companySizes.filter(s => s.slug !== size.slug).map(s => (
                  <Link key={s.slug} href={`/for/${s.slug}`} className="text-xs font-semibold text-[var(--muted)] hover:text-teal-600 bg-[var(--bg)] hover:bg-teal-50 border border-[var(--border)] hover:border-teal-200 rounded-full px-3 py-1.5 transition-all">
                    {s.title}
                  </Link>
                ))}
              </div>
      ...
      <FAQAccordion faqs={defaultFaqs} title={`Spørsmål fra ${size.title.toLowerCase()}`} />
    </>
  )
}
