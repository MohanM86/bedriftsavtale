...
const typeColor: Record<string, string> = {
  kategori: "bg-teal-50 text-teal-700",
  guide: "bg-blue-50 text-blue-700",
  bransje: "bg-purple-50 text-purple-700",
}
        ...
        <div className="container-site py-8">
          ...
          <div className="relative max-w-xl">
            ...
            {query && (
              <button
                onClick={() => setQuery("")}
                className="absolute right-4 top-1/2 -translate-y-1/2 text-[var(--light)] hover:text-[var(--text)]"
              >
                ✕
              </button>
            )}
      ...
      <div className="container-site py-8">
        {!query && (
          <div>
            ...
            {/* Popular searches */}
            <div>
              <h2 className="text-[10px] font-bold tracking-[.1em] uppercase text-[var(--text)] mb-3">Populære søk</h2>
              <div className="flex flex-wrap gap-2">
                {["Mobilabonnement", "Strøm bedrift", "Forsikring", "Drivstoffkort", "Bensinkort", "IT-drift", "Regnskap", "Leasing"].map(term => (
                  <button
                    key={term}
                    onClick={() => setQuery(term)}
                    className="text-xs font-semibold text-teal-600 bg-teal-50 hover:bg-teal-100 border border-teal-100 px-3 py-1.5 rounded-full transition-colors"
                  >
                    {term}
                  </button>
                ))}
        ...
        {results.length > 0 && (
          <div>
            <p className="text-xs text-[var(--muted)] mb-5 font-medium">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`text-[9px] font-bold tracking-wide uppercase px-2 py-0.5 rounded-full ${typeColor[item.type]}`}>
                        {typeLabel[item.type]}
                      </span>
                    </div>
                  ...
                  <span className="text-[var(--light)] text-sm mt-2 flex-shrink-0 group-hover:text-teal-500 transition-colors">→</span>
                </Link>
              ))}
            </div>
          </div>
