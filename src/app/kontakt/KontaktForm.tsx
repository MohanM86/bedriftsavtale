...
export function KontaktForm() {
    ...
    return (
      <div className="bg-teal-50 border border-teal-200 rounded-2xl p-8 flex flex-col items-center text-center gap-4">
        ...
        <p className="text-sm text-[var(--muted)] leading-relaxed max-w-xs">
          Takk for henvendelsen, {form.name.split(" ")[0]}. Vi svarer deg så raskt som mulig.
        </p>
        <button
  ...
  return (
    <div className="bg-teal-500 rounded-2xl p-7">
      ...
      <h2 className="text-xl font-extrabold text-white mb-6 leading-snug tracking-tight">
        Finn riktig bedriftsavtale for din bedrift
      </h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input
              ...
              className="w-full bg-white/15 border border-white/20 rounded-xl px-4 py-3 text-sm text-white placeholder-white/40 focus:outline-none focus:border-white/60 focus:bg-white/20 transition-all"
            />
          </div>
          <div>
        ...
        <div>
          ...
          <textarea
            ...
            onChange={handleChange}
            rows={4}
            placeholder="Beskriv hvilke bedriftsavtaler du ser etter, eller still oss et spørsmål..."
            className="w-full bg-white/15 border border-white/20 rounded-xl px-4 py-3 text-sm text-white placeholder-white/40 focus:outline-none focus:border-white/60 focus:bg-white/20 transition-all resize-none"
          />
        </div>
        ...
        <button
          ...
          className="w-full bg-white hover:bg-teal-50 text-teal-600 font-bold text-sm py-3.5 rounded-full transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
        >
